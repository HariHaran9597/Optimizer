...
- **Data Science**: Time series validation, hyperparameter tuning, model evaluation
- **Machine Learning**: Ridge regression, GridSearchCV, cross-validation strategies
- **Optimization**: Constrained non-linear optimization (SLSQP), objective functions
- **Software Engineering**: Model serialization, error handling, modular code
- **Visualization**: Interactive dashboards, Plotly charts, response curves
- **Production Readiness**: Model versioning, configuration management, reproducibility

## For Interviews

**Q: Why not use random train/test split?**
- A: Time series data violates the i.i.d. assumption. Using TimeSeriesSplit respects temporal order and prevents data leakage.

**Q: Why Ridge regression instead of deep learning?**
- A: Ridge is interpretable, fast, and sufficient for this problem. It allows direct inference of channel coefficients.

**Q: How do you handle diminishing returns?**
- A: Hill saturation curves model realistic spending behavior - more spend doesn't linearly increase returns.

**Q: Why serialize the model?**
- A: Production systems need reproducibility. joblib serialization captures coefficients, features, and saturation parameters.

## Files Reference

| File | Purpose |
|------|---------|
| `src/mmm_utils.py` | Shared functions (adstock, Hill curve, etc.) |
| `src/mmm_model.pkl` | Trained model artifacts (generated by 03_modeling_pro.ipynb) |
| `dashboard/app.py` | Streamlit web interface for optimization |
| `notebooks/03_modeling_pro.ipynb` | Main modeling pipeline (run this first) |

## Troubleshooting

**Model file not found:**
```
Run 03_modeling_pro.ipynb to generate src/mmm_model.pkl
```

**Path errors in dashboard:**
```
cd to root directory and run: streamlit run dashboard/app.py
```

**Data not found:**
```
Run 01_data_generation.ipynb and 02_feature_engineering.ipynb first
```
